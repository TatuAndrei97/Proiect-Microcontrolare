...
SoftwareSerial esp8266(9,10);                        //set the Rx ==> Pin 2; TX ==> Pin3.
...

{
  ...
  xTaskCreate(TaskBlink3, "task3", 128, NULL, 1, &TaskHandle_3 );

  xTaskCreate(TaskBlink4, "task4", 128, NULL, 1, &TaskHandle_4 );
...
void InitWifiModule()
{
  ...
  //delay(1000);
  sendData("AT+CWJAP=\"Tenda_3E3D48\",\"12345678\"\r\n", 2000, DEBUG);        //connect to the WiFi network.
  delay (3000);
...
if(esp8266.available())                                           //if there's any data received and stored in the serial receive buffer, go and excute the if-condition body. If not, dont excute the if-condition body at all.
  {
    ...
    {
      vTaskDelay(100);                                                  //wait 1 second to fill up the buffer with the data.
...
void TaskBlink4 (void *pvParameters) {
  while(1){
   ...
   if (digitalRead(ledPin) == HIGH) {
      ...
      Serial.println("Task4"); // afisare in consola faptul ca task3 a fost realizat
    }
